...
/-!
### Intersection - exactly the same values are in both sets
...
| ⟨ 2 ev_set, _ ⟩ -- proof that 2 is in ev_set and proof 2 is in small_set
...
/-!
### Union - just the combination of the two sets into a larger set
...
example : 7 ∈ ev_set := _  -- no proof for 1 = 0, cannot be proven

example : 7 ∉ ev_set := λ h => nomatch h  -- no cases,

/-!
### Set Complement - elements not in the set
...
example : 6 ∈ ev_set \ small_set := ⟨ rfl, λ p => nomatch p ⟩    -- even set - small set
